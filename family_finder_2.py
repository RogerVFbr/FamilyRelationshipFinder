...
class FamilyFinder:
    ...
    def __get_direct_relatives(self, subject, target_depth, relatives=np.array([]), depth=0):

        if target_depth >= 0:
            acquired_relatives = self.__get__parents(subject)
            depth += 1
        else:
            acquired_relatives = self.__get_children(subject)
            depth -= 1
        ...
        for x in acquired_relatives:
            new_relatives = self.__get_direct_relatives(x, target_depth, relatives, depth)
